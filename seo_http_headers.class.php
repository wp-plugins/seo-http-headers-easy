...
class Seo_Http_Head{
    ...
    private function _set_headers($array){
        $post = get_post(null);
        if(!empty($post)){
            $postDate = $post->post_modified_gmt;
        }else{
            global $wp_query;
        ...
        $LastModifiedUnix = mysql2date('U', $postDate, false);
        if(!is_array($array))return;
        if($array['lm']=='set') header("Last-Modified: " .date('D, d M Y H:i:s',$LastModifiedUnix). " GMT");
        $cctm = '';
