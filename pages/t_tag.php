<?php $option = $this->getOptions['tag'];?>
<table id="settings-headers" cellspacing="0">
    <?php include"tabel_head.php";?>
    <tr>
        <td class="tit" rowspan="5"><p><b>TAG</b>.php</p></td>
        <td class="lab tdfirst"><label for="taglm">Last-Modified</label></td>
        <td class="inp tdfirst">
            <select id="taglm" name="taglm">
        ...
        <td id="tag" class="res" rowspan="5"><?php $this->_print_headers($option);?></td>
    </tr>
    <tr>
        <td class="lab"><label for="tagcc">Cache-Control</label></td>
        <td class="inp">
            <select id="tagcc" name="tagcc">
    ...
    <tr>
        <td class="lab"><label for="tagcctm"><?php _e('Time for Cache', self::slug);?></label></td>
        <td class="inp">
            <input type="text" id="tagcctm" title="<?php printf( __('The entered time must be in the format %sHH:ii:ss%s', self::slug), '&laquo;', '&raquo;' );?>" name="tagcctm" value="<?php echo $option['cctm']; ?>"/>
            <div id="slider-range-min-tag"></div>
        </td>
    </tr>
    <tr>
        <td class="lab tdlast"><label for="tagping">X-Pingback</label></td>
        <td class="inp tdlast">
            <select id="tagping" name="tagping">
